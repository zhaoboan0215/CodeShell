...
from codeshell.commands.parser import complete, parse_command
from codeshell.commands.registry import (
    Command,
    CommandContext,
    CommandRegistry,
    CommandType,
...
class MockUI:
    def __init__(self) -> None:
        ...
        self.available_models = ("deepseek-flash", "deepseek-v4-pro")
        self.current_model = "deepseek-flash"
        self.model_selector_calls: list[tuple[tuple[str, ...], str]] = []
        self.model_switch_calls: list[str] = []
    ...
    def show_model_selector(
        ...
        self.model_selector_calls.append((models, current))

    async def switch_model(self, model: str) -> bool:
        self.model_switch_calls.append(model)
        return True
...
class TestHelpHandler:
    ...
    async def test_list_all(self) -> None:
        ...
        from codeshell.commands.handlers.help import handle_help
...
class TestPlanDoHandlers:
    ...
    async def test_plan_switches_mode(self) -> None:
        from codeshell.commands.handlers.plan import handle_plan
...
class TestModelHandler:
    @pytest.mark.asyncio
    async def test_model_without_args_opens_selector(self) -> None:
        from codeshell.commands.handlers.model import handle_model
        ...
        assert ui.model_selector_calls == [
            (("deepseek-flash", "deepseek-v4-pro"), "deepseek-flash")
        ]
        assert ui.model_switch_calls == []

    @pytest.mark.asyncio
    async def test_model_with_arg_switches_directly(self) -> None:
        from codeshell.commands.handlers.model import handle_model

        ui = MockUI()
        ctx = _make_context(args="deepseek-v4-pro", ui=ui)

        await handle_model(ctx)

        assert ui.model_switch_calls == ["deepseek-v4-pro"]
        assert ui.model_selector_calls == []

    @pytest.mark.asyncio
    async def test_model_with_multiple_args_shows_usage(self) -> None:
        from codeshell.commands.handlers.model import handle_model

        ui = MockUI()
        ctx = _make_context(args="deepseek-flash extra", ui=ui)

        await handle_model(ctx)

        assert ui.model_switch_calls == []
        assert ui.messages == [
            "用法: /model [deepseek-flash|deepseek-v4-pro]"
        ]

    @pytest.mark.asyncio
    async def test_help_model_describes_both_modes(self) -> None:
        from codeshell.commands.handlers import register_all_commands
        from codeshell.commands.handlers.help import handle_help
        ...
        register_all_commands(registry)
        ui = MockUI()
        ctx = _make_context(args="model", ui=ui)
        ctx.config = {"registry": registry}

        await handle_help(ctx)

        assert "/model" in ui.messages[0]
        assert "deepseek-flash" in ui.messages[0]
        assert "deepseek-v4-pro" in ui.messages[0]
        assert "/model [" in ui.messages[0]

class TestSkillHandler:
    @pytest.mark.asyncio
    async def test_skill_list_no_loader(self) -> None:
        from codeshell.commands.handlers.skill import handle_skill
...
class TestStatusHandler:
    ...
    async def test_status_output(self) -> None:
        ...
        assert "CodeShell 状态" in ui.messages[0]
        assert "default" in ui.messages[0]

class TestSessionHandler:
    @pytest.mark.asyncio
    async def test_session_no_manager(self) -> None:
        from codeshell.commands.handlers.session import handle_session
...
class TestMemoryHandler:
    ...
    async def test_memory_display(self) -> None:
        from codeshell.commands.handlers.memory import handle_memory
...
class TestRegisterAllCommands:
    ...
    def test_model_command_is_available_to_completion(self) -> None:
        from codeshell.commands.handlers import register_all_commands
        ...
        assert any(
            value == "/model"
            for _display, value in complete(registry, "/mod")
        )
