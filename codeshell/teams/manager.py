...
from codeshell.teams.backend_detect import BackendDetectionError, detect_backend
from codeshell.teams.mailbox import Mailbox, create_message
...
from codeshell.teams.shared_task import SharedTaskStore
from codeshell.teams.spawn_inprocess import InProcessTeammateHandle
    ...
    def _kill_pane(self, pane_id: str, backend_type: str) -> None:
        try:
            if backend_type == BackendType.TMUX.value:
                from codeshell.teams.spawn_tmux import kill_pane
                kill_pane(pane_id)
            elif backend_type == BackendType.ITERM2.value:
                from codeshell.teams.spawn_iterm2 import kill_pane
                kill_pane(pane_id)
