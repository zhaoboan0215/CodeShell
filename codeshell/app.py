...
from codeshell.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
...
from codeshell.hooks import HookContext, HookEngine, load_hooks
from codeshell.conversation import ConversationManager, Message
from codeshell.mcp import ConnectResult, MCPManager
from codeshell.mcp.tool_wrapper import mcp_tool_name_prefix
from codeshell.memory import (
    MemoryManager,
    Session,
    SessionManager,
    RecallResult,
...
from codeshell.model_switching import (
    ...
    get_available_models as get_switchable_deepseek_models,
...
from codeshell.agents.notification import inject_task_notifications
from codeshell.commands.handlers.tasks import create_tasks_command
from codeshell.skills.executor import SkillExecutor
from codeshell.skills.loader import SkillLoader
from codeshell.commands.handlers.skill_register import register_skill_commands
...
from codeshell.tools.ask_user import AskUserEvent, AskUserTool
...
from codeshell.worktree.cleanup import start_stale_cleanup_task
from codeshell.worktree.manager import WorktreeManager
from codeshell.commands.handlers.worktree import create_worktree_command
from codeshell.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".codeshell", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".codeshell" / "history"
...
class CodeShellApp(App):
    ...
    def __init__(
        ...
        from codeshell.config import SandboxAppConfig
    ...
    def _make_banner(model: str = "", work_dir: str = "") -> RichText:
        ...
        t.append(f"CodeShell v{__version__}\n", style="color(242)")
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="input-area"):
            with Vertical(id="model-select"):
                yield Static("Switch DeepSeek model", id="model-select-label")
    ...
    def on_mount(self) -> None:
        self.register_theme(_CODESHELL_THEME)
        self.theme = "codeshell"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        from codeshell.tools.exit_plan_mode import ExitPlanModeTool
        ...
        from codeshell.commands.handlers.trace import create_trace_command
    ...
    def on_option_list_option_selected(self, event: OptionList.OptionSelected) -> None:
        if event.option_list.id == "provider-list":
            ...
            model = str(event.option.id or "")
            self._hide_model_selector()
            if model:
                self.run_worker(self.switch_model(model), exclusive=False)
    ...
    def get_available_models(self) -> tuple[str, ...]:
        if self._selected_provider is None:
            return ()
        return get_switchable_deepseek_models(self._selected_provider)
    ...
    def show_model_selector(
        ...
        if self._streaming:
            self._show_system_message(
                "当前回复仍在生成，请等待完成后再切换模型。"
            )
            return
        if not models:
            self._show_system_message(
                "当前 Provider 不支持运行时模型切换。"
            )
            return
        ...
        option_list.add_options(
            [
                Option(
                    f"✓ {model}  (current)" if model == current else f"  {model}",
                    id=model,
                )
                for model in models
            ]
    ...
    def _hide_model_selector(self) -> None:
    ...
    async def switch_model(self, model: str) -> bool:
        ...
        if current is None or self.client is None or self.agent is None:
            self._show_system_message("模型尚未初始化，当前无法切换。")
            return False
        ...
        if model == current.model:
            self._show_system_message(f"{model} 已是当前模型，无需切换。")
            return False
        if model not in available:
            self._show_system_message(
                f"不支持模型 {model}。可用模型: "
                + ", ".join(SUPPORTED_DEEPSEEK_MODELS)
            )
            return False

        try:
            prepared = prepare_model_switch(current, model)
        except Exception:
            self._show_system_message(
                f"模型切换失败，已继续使用 {current.model}。"
            )
            return False
        ...
        old_skill_state = (
            (self.skill_executor.client, self.skill_executor.protocol)
            if self.skill_executor is not None
            else None
        )
        old_agent_tool_provider = current

        try:
            self._selected_provider = prepared.provider
            self.client = prepared.client
            self.agent.set_runtime_client(
                prepared.client,
                prepared.provider.protocol,
                prepared.context_window,
            )
            if self.skill_executor is not None:
                self.skill_executor.set_runtime_client(
                    prepared.client,
                    prepared.provider.protocol,
                )
            if self._agent_tool is not None:
                self._agent_tool.set_provider_config(prepared.provider)
            self._update_model_display()
        except Exception:
            self._selected_provider = current
            self.client = old_client
            self.agent.set_runtime_client(*old_agent_state)
            if self.skill_executor is not None and old_skill_state is not None:
                self.skill_executor.set_runtime_client(*old_skill_state)
            if self._agent_tool is not None:
                self._agent_tool.set_provider_config(old_agent_tool_provider)
            self._update_model_display()
            self._show_system_message(
                f"模型切换失败，已继续使用 {current.model}。"
            )
        ...
        self._show_system_message(f"已切换模型: {prepared.provider.model}")
        return True

    def _update_model_display(self) -> None:
        model = self.get_current_model()
        try:
            self.query_one("#model-label", Static).update(model)
            work_dir = self.agent.work_dir if self.agent is not None else os.getcwd()
            self.query_one("#title-bar", Static).update(
                self._make_banner(model, work_dir)
            )
            self._update_mode_label()
        except Exception:
            pass
    ...
    async def _show_plan_approval(self) -> None:
        from codeshell.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from codeshell.plan_dialog import InlinePlanWidget, PlanChoice
        from codeshell.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from codeshell.askuser_dialog import InlineAskUserWidget
