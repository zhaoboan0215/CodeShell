...
def validate_providers(raw_providers: list) -> list[dict]:
    ...
    for i, entry in enumerate(raw_providers):
        ...
        if not isinstance(raw_models, list):
            raise ConfigError(f"Provider #{i + 1}: models must be a list")
        if raw_models:
            if any(not isinstance(model, str) or not model.strip() for model in raw_models):
                raise ConfigError(
                    f"Provider #{i + 1}: models must contain non-empty strings"
                )
            if len(set(raw_models)) != len(raw_models):
                raise ConfigError(f"Provider #{i + 1}: models must not contain duplicates")
            if entry["model"] not in raw_models:
                raise ConfigError(
                    f"Provider #{i + 1}: default model '{entry['model']}' "
